...
async def send_rebooking_suggestions(bot: Bot):
    """Через день после завершённой сессии предлагаем записаться снова."""
    from datetime import datetime, timedelta
    async with __import__("storage")._pool.acquire() as conn:
        rows = await conn.fetch("""
            SELECT b.user_id, b.name, b.lang, cs.completed_at
    ...
    for r in rows:
        lang = r["lang"]
        kb = __import__("aiogram.utils.keyboard", fromlist=["InlineKeyboardBuilder"]).InlineKeyboardBuilder()
        kb.button(
        ...
        kb.adjust(1)
        text = (
            f"👋 {r['name']}, как вы себя чувствуете после сессии?\n\n"
            "Если хотите продолжить работу — запишитесь на следующую встречу. 💙"
            if lang == "ru" else
            f"👋 {r['name']}, як ви почуваєтеся після сесії?\n\n"
