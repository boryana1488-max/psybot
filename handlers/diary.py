...
async def diary_start(callback: CallbackQuery, state: FSMContext):
    ...
    if lang == "uk":
        text = "📓 Щоденник відчуттів\n\n🔍 Крок 1/4: Де в тілі найбільше відчуттів?"
        markup = kb([
            ("Голова/Шия", "dl_Голова/Шия"), ("Груди", "dl_Груди"),
            ("Живіт", "dl_Живіт"), ("Спина", "dl_Спина"),
            ("Руки/Ноги", "dl_Руки/Ноги"), ("Все тіло", "dl_Все тіло"),
        ])
    else:
        text = ("📓 Дневник ощущений\n\n🔍 Шаг 1/4: Где в теле больше всего ощущений?" if lang == "ru" else "📓 Щоденник відчуттів\n\n🔍 Крок 1/4: Де в тілі найбільше відчуттів?")
        markup = kb([
            (("Голова/Шея", "dl_Голова/Шея") if lang=="ru" else ("Голова/Шия", "dl_Голова/Шея")), (("Грудь","dl_Грудь") if lang=="ru" else ("Груди","dl_Грудь")),
            (("Живот","dl_Живот") if lang=="ru" else ("Живіт","dl_Живот")), (("Спина","dl_Спина") if lang=="ru" else ("Спина","dl_Спина")),
            (("Руки/Ноги","dl_Руки/Ноги") if lang=="ru" else ("Руки/Ноги","dl_Руки/Ноги")), (("Всё тело","dl_Всё тело") if lang=="ru" else ("Все тіло","dl_Всё тело")),
        ])
...
async def diary_location(callback: CallbackQuery, state: FSMContext):
    ...
    lang = (await state.get_data()).get("lang", "ru")
    if lang == "uk":
        await safe_edit(callback, "💭 Крок 2/4: Як це відчувається?", kb([
            ("Стиснення", "ds_Стиснення"), ("Жар/Холод", "ds_Жар/Холод"),
            ("Порожнеча", "ds_Порожнеча"), ("Важкість", "ds_Важкість"),
            ("Поколювання", "ds_Поколювання"), ("Пульсація", "ds_Пульсація"),
            ("Оніміння", "ds_Оніміння"), ("Грудка в горлі", "ds_Грудка в горлі"),
        ]))
    else:
        await safe_edit(callback, ("💭 Шаг 2/4: Как это ощущается?" if lang == "ru" else "💭 Крок 2/4: Як це відчувається?"), kb([
...
async def diary_sensation(callback: CallbackQuery, state: FSMContext):
    ...
    b.adjust(5)
    text = ("📊 Крок 3/4: Інтенсивність від 1 (ледве) до 10 (нестерпно):" if lang == "uk"
            else ("📊 Шаг 3/4: Интенсивность от 1 (едва) до 10 (невыносимо):" if lang == "ru" else "📊 Крок 3/4: Інтенсивність від 1 (ледве) до 10 (нестерпно):"))
...
async def diary_intensity(callback: CallbackQuery, state: FSMContext):
    ...
    lang = (await state.get_data()).get("lang", "ru")
    if lang == "uk":
        await safe_edit(callback, "❤️ Крок 4/4: Що ви відчуваєте?", kb([
            ("Тривога", "de_Тривога"), ("Гнів", "de_Гнів"),
            ("Радість", "de_Радість"), ("Втома", "de_Втома"),
            ("Сум", "de_Сум"), ("Спокій", "de_Спокій"),
        ]))
    else:
        await safe_edit(callback, ("❤️ Шаг 4/4: Что вы чувствуете?" if lang == "ru" else "❤️ Крок 4/4: Що ви відчуваєте?"), kb([
...
async def diary_emotion(callback: CallbackQuery, state: FSMContext):
    ...
    sens = data.get("sensation", "")
    intens = data.get("intensity", "")
    body_line = "📍 " + str(loc) + " | " + str(sens) + " | " + str(intens) + "/10 | " + str(emotion)
    if lang == "uk":
        summary = "✅ Записано!\n\n" + body_line + "\n\nДякуємо — це допомагає краще зрозуміти себе. 💙"
        back_btn = "◀️ Головне меню"
    else:
        summary = "✅ Записано!\n\n" + body_line + "\n\nСпасибо — это помогает лучше понять себя. 💙"
        back_btn = "◀️ Главное меню"

    b = InlineKeyboardBuilder()
    b.button(text=back_btn, callback_data="main_menu")
