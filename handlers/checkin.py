...
async def checkin_toggle(callback: CallbackQuery, state: FSMContext):
    ...
    if storage.is_checkin_subscribed(uid):
        storage.unsubscribe_checkin(uid)
        text = ("🔕 Вечерний чек-ин отключён." if lang == "ru"
                else "🔕 Вечірній чек-ін вимкнено.")
    else:
        storage.subscribe_checkin(uid)
...
async def my_progress(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    lang = data.get("lang", "ru")
    history = storage.get_user_mood_history(callback.from_user.id)
...
async def checkin_answer(message: Message, state: FSMContext):
    ...
    storage.save_checkin(user_id=message.from_user.id, name=name, text=message.text.strip())
...
async def checkin_loop(bot: Bot):
    while True:
        ...
        for user_id in list(storage.get_subscribed_users()):
