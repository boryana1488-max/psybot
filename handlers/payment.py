...
async def pay_consultation(callback: CallbackQuery, state: FSMContext):
    ...
    kb = paid_kb(lang, tx_id)

    photo = get_image("payment")
    if photo:
        try:
...
async def course_detail(callback: CallbackQuery, state: FSMContext):
    ...
    if not course:
        await callback.answer("Курс не найден." if data.get("lang","ru") == "ru" else "Курс не знайдено.", show_alert=True)
        return
...
async def payment_reminder_loop(bot: Bot):
    ...
    while True:
        ...
        now = datetime.now()
        for b in await storage.get_all_bookings():
