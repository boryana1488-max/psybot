...
def tz_kb(lang: str):
    kb = InlineKeyboardBuilder()
    kb.button(text="🇺🇦 Украина" if lang == "ru" else "🇺🇦 Україна", callback_data="tz_ua")
    kb.button(text="🇪🇺 Европа" if lang == "ru" else "🇪🇺 Європа", callback_data="tz_eu")
    kb.button(text="🌎 Сев./Юж. Америка" if lang == "ru" else "🌎 Пн./Пд. Америка", callback_data="tz_am")
    kb.button(text="🌏 Азия" if lang == "ru" else "🌏 Азія", callback_data="tz_as")
    kb.adjust(2)
    return kb.as_markup()
...
async def start_booking(callback: CallbackQuery, state: FSMContext):
    ...
    for date_key in sorted(by_date.keys()):
        ...
        # Извлекаем дату+день для кнопки: "15.04 Пятница"
        parts = slot_str.split(" ")
