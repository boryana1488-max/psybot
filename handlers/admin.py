...
async def adm_booking_detail(callback: CallbackQuery):
    ...
    b = storage.get_booking_by_index(n)
    if not b:
...
async def adm_booking_slot_set(callback: CallbackQuery):
    ...
    user_id = b["user_id"]
    lang = b.get("lang", "ru")
    storage.change_booking_slot(b, new_slot)
...
async def reschedule_approve(callback: CallbackQuery):
    ...
    pending = storage.get_pending_reschedules()
    if idx >= len(pending):
        await callback.answer("Запрос уже обработан.", show_alert=True)
        return
    req = pending[idx]
    b = storage.get_booking_by_user(req["user_id"])
    if b:
        storage.change_booking_slot(b, req["new_slot"])
    storage.resolve_reschedule(req)
    await callback.message.edit_text(
...
async def adm_analytics(callback: CallbackQuery):
    ...
    users = storage.get_all_users()
    reviews = storage.get_all_reviews()
    mood = storage.get_mood_entries()
    diary = storage.get_diary_entries()
    checkins = storage.get_checkin_entries()

    avg_rating = (sum(r["rating"] for r in reviews) / len(reviews)) if reviews else 0
    avg_mood = (sum(e["mood"] for e in mood) / len(mood)) if mood else 0
    slots_total = len(storage.get_all_slots())
...
async def adm_notes_list(callback: CallbackQuery):
    ...
    for uid, info in users.items():
        note_icon = "📝" if storage.get_note(uid) else "👤"
...
async def adm_note_view(callback: CallbackQuery):
    ...
    info = storage.get_all_users().get(uid, {})
    note = storage.get_note(uid)
...
async def adm_note_save(message: Message, state: FSMContext):
    ...
    storage.set_note(uid, message.text.strip())
    await state.clear()
...
async def adm_checkins(callback: CallbackQuery):
    if not is_admin(callback.from_user.id): return
    entries = storage.get_checkin_entries()
    if not entries:
