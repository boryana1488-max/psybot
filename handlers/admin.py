...
def booking_actions_kb(n: int):
    ...
    kb.button(text="🏁 Завершить сессию", callback_data=f"adm_bk_complete_{n}")
    kb.button(text="◀️ Назад", callback_data="adm_bookings")
    kb.adjust(2, 2, 1)
    return kb.as_markup()


def slot_dates_kb(n: int, free_slots: list):
    ...
    for date_key in sorted(by_date.keys()):
        ...
        kb.button(text=f"📅 {label}", callback_data=f"adm_bkd_{n}_{date_key}")
    kb.button(text="◀️ Назад", callback_data="adm_bookings")
    kb.adjust(2)
    return kb.as_markup(), dict(by_date)
...
_adm_slot_cache: dict = {}  # n -> {date_key -> [slots]}
...
async def adm_booking_change(callback: CallbackQuery):
    ...
    if not free:
        ...
        return
    kb, by_date = slot_dates_kb(n, free)
    _adm_slot_cache[n] = by_date
    await callback.message.edit_text(
        f"Выберите дату для записи #{n}:",
        reply_markup=kb
    )

@router.callback_query(F.data.startswith("adm_bkd_"))
async def adm_booking_date_chosen(callback: CallbackQuery):
    if not is_admin(callback.from_user.id): return
    parts = callback.data[8:].rsplit("_", 1)
    n = int(parts[0])
    date_key = parts[1]
    by_date = _adm_slot_cache.get(n, {})
    ...
    for i, s in enumerate(day_slots):
        ...
        kb.button(text=f"🕐 {time_part}", callback_data=f"adm_bkst_{n}_{date_key}_{i}")
    kb.button(text="◀️ Назад", callback_data=f"adm_bk_change_{n}")
    kb.adjust(2)
    await callback.message.edit_text(f"Выберите время для записи #{n}:", reply_markup=kb.as_markup())

@router.callback_query(F.data.startswith("adm_bkst_"))
async def adm_booking_slot_set(callback: CallbackQuery):
    if not is_admin(callback.from_user.id): return
    raw = callback.data[9:]
    parts = raw.rsplit("_", 1)
    idx = int(parts[1])
    dn_parts = parts[0].rsplit("_", 1)
    n = int(dn_parts[0])
    date_key = dn_parts[1]
    by_date = _adm_slot_cache.get(n, {})
    day_slots = by_date.get(date_key, [])
...
@router.callback_query(F.data.startswith("adm_rmi_"))
async def adm_rmslot(callback: CallbackQuery):
    global _rm_slot_cache
    if not is_admin(callback.from_user.id): return
    idx = int(callback.data[8:])
    if idx >= len(_rm_slot_cache):
        await callback.answer("Слот не найден.", show_alert=True)
        return
    slot = _rm_slot_cache[idx]["slot"]
...
async def adm_note_view(callback: CallbackQuery):
    ...
    kb.button(text="📊 Карточка клиента", callback_data=f"adm_client_card_{uid}")
    kb.button(text="💬 Написать клиенту", callback_data=f"adm_reply_{uid}")
...
async def adm_client_card(callback: CallbackQuery):
    ...
    kb = InlineKeyboardBuilder()
    kb.button(text="✏️ Заметка", callback_data="adm_note_edit_" + str(uid))
    kb.button(text="💬 Написать", callback_data="adm_reply_" + str(uid))
    kb.button(text="◀️ Назад", callback_data="adm_note_view_" + str(uid))
    kb.adjust(2, 1)
    await callback.message.edit_text(text, reply_markup=kb.as_markup())
...
async def adm_complete_session(callback: CallbackQuery):
    if not is_admin(callback.from_user.id): return
    n = int(callback.data.split("_")[3])
    b = await storage.get_booking_by_index(n)
    if not b:
    ...
    lang = b.get("lang", "ru")
    await callback.message.edit_text("✅ Сессия завершена!", reply_markup=back_admin_kb())

    # Клиенту: запрос отзыва
...
async def adm_reply_ask(callback: CallbackQuery, state: FSMContext):
    ...
    await state.update_data(reply_target_uid=uid)
    await state.set_state(ReplyState.typing)
    info = (await storage.get_all_users()).get(uid, {})
    name = info.get("name", str(uid))
    await callback.message.edit_text(
        f"Напишите сообщение для {name}:",
