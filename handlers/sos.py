...
async def sos_step1(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    lang = data.get("lang", "ru")
    await callback.message.edit_text(t(lang, "sos_step1"), reply_markup=next_kb(lang, "sos_step2"))
...
async def sos_step2(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    lang = data.get("lang", "ru")
    await callback.message.edit_text(t(lang, "sos_step2"), reply_markup=next_kb(lang, "sos_step3"))
...
async def sos_step3(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
    lang = data.get("lang", "ru")
    await callback.message.edit_text(t(lang, "sos_step3"), reply_markup=sos_final_kb(lang))
