...
async def save_questionnaire(user_id: int, answers: dict):
    """answers = {"goal": ..., "request": ..., "source": ...}"""
    import json
    async with _conn() as conn:
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS questionnaires (
                user_id BIGINT PRIMARY KEY,
                goal TEXT, request TEXT, source TEXT
            )
        """)
        await conn.execute("""
...
async def has_questionnaire(user_id: int) -> bool:
    q = await get_questionnaire(user_id)
...
_rate_cache: dict[int, list] = {}  # user_id -> [timestamps]

def check_rate_limit(user_id: int, max_calls: int = 10, window_sec: int = 60) -> bool:
